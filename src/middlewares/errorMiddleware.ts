...
const errorMiddleware = (error: HttpException, request: Request, response: Response, next: NextFunction) => {
  ...
  const errorResponse = new HttpException(status, message, errorCode, validationErrors);
  errorResponse.response = additionalInfo;
  if (status === 500) {
  ...
  response.status(status).send(fmt.formatResponse(errorResponse, 0, additionalInfo?.message || message));
};
